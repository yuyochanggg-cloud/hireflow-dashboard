...
# ── 01_職缺主檔欄名（2026-08-04補：這張表一直沒有集中定義，app.py的
# sync_library_to_gsheet只同步02/03/04，職缺本身從沒被自動寫進Sheets過——
# 新建職缺→跑篩選→寄推薦信全部正常，但招募看板/首頁看不到，因為01根本沒有
# 這筆職缺的列。欄名照Sheets實際表頭校正，不是憑空定義。）──
S1_COLS = [
    "job_id", "職缺名稱", "部門", "工作地點",
    "必備條件（摘要）", "加分條件（摘要）", "評分維度（JSON）",
    "狀態", "建立日期", "最後更新", "備註",
]
# 「部門」「建立日期」「備註」只在新建列時給預設值，更新既有職缺時不覆寫
# （部門本來就沒有資料來源會填、備註可能被人工填過、建立日期只該在第一次
# 建立時寫入）；「狀態」不保護——它要忠實反映job_status這個單一權威來源，
# 每次同步都該覆寫成當下狀態，不是「第一次寫入後就不能變」的欄位。
S1_PROTECT_ON_UPDATE = ["部門", "建立日期", "備註"]
...
# 02_候選人主檔更新既有列時不覆寫這些欄——「初次進庫日期」只在candidate_id
# 真正第一次出現時才該寫入，重複同步同一人（例如整批重新評分/資料同步）
# 不該把它改成「最後同步日期」（那是「最後更新日期」欄的職責，兩者不能混用）。
S2_PROTECT_ON_UPDATE = ["初次進庫日期"]
S3_COLS = [
...
S3_PROTECT_ON_UPDATE = [
    ...
    "應徵批次日期",  # 2026-07-27修正：這欄本該是「這筆應徵真正發生的日期」，
    # 但沒被保護清單擋住時，每次同步都被_build_master_rows蓋成「今天」，
    # 導致831筆裡日期只剩4種值，分析報表的新進候選人數字完全失真。
]

# 結案原因選項（單一來源，看板/候選人頁的結案按鈕共用）
CLOSE_REASONS = ["訊息未回", "主管不邀約", "面試未到", "面試未通過", "候選人婉拒"]
# 選了這些原因，結案時順便同步一筆05_面試主檔紀錄，避免分析報表的面試通過率
# 又因為沒人回頭填記分卡而失真。value = (面試結果, 面試官備註)。
# 2026-08-12 修正：「面試未到」以前也寫成「未通過」，但沒出席的人根本沒被評估過，
# 卻會進面試通過率的分母把通過率壓低。改寫成獨立的「面試未到」。
CLOSE_REASON_TO_INTERVIEW = {
    "面試未到": ("面試未到", "no-show，約定面試未出席"),
    "面試未通過": ("未通過", "面試未通過"),
}
S4_COLS = [
...
# 「面試未到」（no_show）是 2026-08-12 獨立出來的類別：以前約了沒出席的人只能記
# 成「未通過」，但那會進面試通過率的分母，把通過率壓低——沒出席根本沒被評估過，
# 不該算成一次面試。no_show 的人流程狀態**維持在約定面試**、不推進到已面試，
# 所以漏斗的「進行面試」也不會把他算進去。
RESULT_MAP = {
    "通過": "pass", "pass": "pass",
    "未通過": "fail", "fail": "fail",
    "面試未到": "no_show", "no_show": "no_show",
    "待定": "pending",
}
